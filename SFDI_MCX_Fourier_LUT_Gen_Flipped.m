...
if multilayerflag == 1
    filename = "SFDI_multilayer_drefval_2layermodelBCthin_nphoton_" + num2str(cfg.nphoton, '%.1e') + "_voxel_" + num2str(cfg.unitinmm) + "mm_flipped.csv";
end 
if multilayerflag == 0
...
mua_dynamic = linspace(0,9.9,100); % what absorbance values do we want to iterate through?

musp_dynamic = linspace(0,29.7,100); % what scattering values do we want to iterate through?
...
%                     musp    = 0.628212 * (1-g);
                    ...
                    layer3  = [mua, mus, g, n]; % middle layer (e.g. dermis)
                else
                    layer3  = [mua_dynamic(j), mus_cur, g, n]; % middle layer (e.g. dermis)
                end
                
                if multilayerflag == 1 && SACflag == 1
                    layer3  = epiderm_laylit; % middle layer (e.g. dermis)
                end
                
                if multilayerflag == 1 && SACflag == 2
                    layer3  = epiderm_laydrk; % middle layer (e.g. dermis)
                end

                layer2      = [mua_dynamic(j)*0.1, mus_cur*0.1, g, n]; % outermost layer (e.g. epidermis OR epidermis+dermis)
                layer4      = [mua, mus, g, n]; % semi-infinite layer (e.g. adipose, tumor, etc.)
...
LUT_name = 'my_LUT_Fourier_flipped' + layer_info + n_sample;

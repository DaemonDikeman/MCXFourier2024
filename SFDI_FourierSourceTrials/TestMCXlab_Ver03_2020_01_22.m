...
% % % % caxis([0,1e-5])
% % hs=slice(log10(abs(double(fcw1))),1,1,60);
% % caxis([-6 -5])
hs=slice(log10(abs(double(fcw1))),1,1,10);
caxis([-6 -5])
...
% % caxis([0,1e-5])
% % hs=slice(log10(abs(double(fcw2))),1,1,60);
% % caxis([-6 -5])
hs=slice(log10(abs(double(fcw2))),1,1,10);
caxis([-6 -5])
set(hs,'linestyle','none');
axis equal; colorbar;box on;
...
% %     f2(i) = mean2(fcw2(:,:,60 - (i-1)));
    f1(i) = mean2(fcw1(:,:,10 - (i-1)));
    f2(i) = mean2(fcw2(:,:,10 - (i-1)));
end
figure
plot(f1);
% % plot(log10(f1));

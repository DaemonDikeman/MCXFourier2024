...
addpath(genpath('G:\My Drive\Molecular Biophotonics Lab\Code\MCX_MMC'))
...
for ii = 1:numfx
    ...
    cfg(ii).prop=[0 0 1 1;0.005 8 0.9 1.4];
    ...
    cfg(ii).srcdir=[0 0 -1];
    cfg(ii).srcparam1=[xy 0 0 fx(ii)*cfg(ii).unitinmm*xy+1/3];
...
% %     cfg(ii).isspecular = 1; % 1-calculate specular reflection if source is outside, [0] no specular reflection
    cfg(ii).isnormalized = 0; % [1]-normalize the output fluence to unitary source, 0-no reflection
